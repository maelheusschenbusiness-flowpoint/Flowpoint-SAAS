/**
 * FlowPoint Onboarding — Step 1 V3
 *
 * Scénario : 30-45s de démonstration pédagogique avec vrais clics.
 *
 ...
 *
 * Résolution : 1920×1080 30fps (capture native Playwright vérifiée).
 * Caméra : fixe, aucun pan, aucun zoom FFmpeg.
 * Fade in : 0.2s (quasi-immédiat).
 */
...
async function scenario(page, token) {
  ...
  await dwell(page, kpi1Pos.x, kpi1Pos.y, 2800, 550);
  mark('beat2_kpi_done');
  ...
  await dwell(page, listCenter.x, listCenter.y, 2500, 400);
  ...
  if (auditRowPos) {
    ...
    await Promise.race([
      page.waitForSelector('.fp-panel,.fp-detail-panel,.fp-modal,.fp-sidebar-panel,[data-panel],[role="dialog"]', { timeout: 5000 }).catch(() => null),
  ...
  if (panelPos) {
    await dwell(page, panelPos.x, panelPos.y, 4000, 400);
  } else {
  ...
  await dwell(page, monPos.x, monPos.y, 3200, 520);
  mark('beat9_done');
...
function postProcess(webm, skipMs) {
  ...
  const out = join(OUT_DIR, 'v1-v3-final.mp4');
  ...
  const fadeInDur  = 0.20;   // ← fade quasi-immédiat (demande utilisateur)
  ...
  console.log(`  FFmpeg étape 2 : ${callouts.length} callout(s) + fades → ${out}`);
  r = spawnSync('ffmpeg', [
    '-y', '-i', raw,
    '-vf', vf,
...
console.log(`▶ Enregistrement V1-V3 à ${W}×${H} (scénario avec vrais clics)…`);

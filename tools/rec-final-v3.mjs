/**
 * V3 final — Monitoring détail + uptime + latence + Incidents + Alertes
 * Fix: sidebar click pour naviguer vers monitors + attente DOM correcte
 */
...
const TMP = '/tmp/rec-fv3'; mkdirSync(TMP, { recursive: true });
...
async function sidebarNav(p, route) {
  await p.evaluate(r => {
    ...
    if (el) el.click(); else if (window.navigate) window.navigate(r);
  }, route);
...
async function hov(p, s, max=3, d=270) {
  ...
  for(let i=0;i<n;i++){const b=await it.nth(i).boundingBox().catch(()=>null);if(b&&b.x>150){await go(p,b.x+b.width/2,b.y+b.height/2,290);await p.waitForTimeout(d);}}
}
...
// Navigate to Monitors via sidebar JS click
...
// === 1. Monitors list — hover cards to show DOWN status ===
await hov(page, '.fp-monitor-card,.fp-monitor-row', 4, 300);

// === 2. Open monitor detail — click first card ===
...
if (fcBox && fcBox.x > 150) {
  await go(page, fcBox.x + fcBox.width*0.5, fcBox.y + fcBox.height/2, 300);
  ...
  await page.evaluate(()=>{
    ...
    if(m&&window.openFloatPanel&&window.renderMonitorDetailPanel)window.openFloatPanel('Détail monitor',window.renderMonitorDetailPanel(m));
  });
...
if (pb) {
  // Show uptime & latence stat cards
  await go(page, pb.x+50, pb.y+100, 240); await page.waitForTimeout(400);
  await go(page, pb.x+pb.width*0.4, pb.y+110, 230); await page.waitForTimeout(450);
  await go(page, pb.x+pb.width*0.7, pb.y+110, 230); await page.waitForTimeout(450);
  // Scroll to see historique
  await sc(page, 100, 380); await page.waitForTimeout(500);
  await go(page, pb.x+pb.width/2, pb.y+180, 230); await page.waitForTimeout(400);
  await sc(page, 80, 330); await page.waitForTimeout(400);
}

// === 4. Click Incidents tab ===
const incClicked = await cl(page, '.fp-tab:has-text("Incidents"),button:has-text("Incidents"),[role="tab"]:has-text("Incidents")', 700, 260);
if (incClicked) {
  await page.waitForTimeout(400);
  // Show the incident row
  const inc = page.locator('.fp-incident-item,.fp-incident-row,tr').first();
  const ib = await inc.boundingBox().catch(()=>null);
  if (ib && ib.x > 150) { await go(page, ib.x+ib.width/2, ib.y+ib.height/2, 260); await page.waitForTimeout(500); }
}
...
// === 6. Navigate to Alertes (alerts-center) via sidebar ===
await sidebarNav(page, 'alerts-center');
await page.waitForTimeout(1000);
await inj(page);

// === 7. Show KPI + alert items ===
await hov(page, '.fp-stat-card,.fp-kpi-card', 4, 260);
await hov(page, '.fp-alert-item,.fp-alert-row', 3, 280);

// === 8. Click first alert item ===
...
if (await alertItems.count() > 0) {
  ...
  if (ab && ab.x > 150) {
    await go(page,ab.x+ab.width/2,ab.y+ab.height/2,260);await page.waitForTimeout(180);
    await page.mouse.click(ab.x+ab.width/2,ab.y+ab.height/2);await page.waitForTimeout(600);await inj(page);
  }
}

// === 9. Scroll to show more alert content ===
await sc(page, 100, 350);
await hov(page, '.fp-alert-rule-row', 2, 260);
...
console.log(`✓ V3  step3-monitoring-alerts.mp4  ${parseFloat(dur).toFixed(1)}s`);
mkdirSync('/tmp/fv3',{recursive:true});
for(const t of[1,4,8,13,18,23,28])spawnSync('ffmpeg',['-y','-ss',String(t),'-i',mp4,'-frames:v','1','-vf','scale=640:360',`/tmp/fv3/t${t}.jpg`],{stdio:'pipe'});
console.log('V3 frames ok');

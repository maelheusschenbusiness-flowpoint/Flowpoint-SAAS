...
router.post("/connectors/:provider/connect", async (req: Request, res: Response) => {
...
router.post("/connectors/:provider/disconnect", async (req: Request, res: Response) => {
  const { provider } = req.params;
  try {
...
router.post("/connectors/:provider/sync", async (req: Request, res: Response) => {
  const { provider } = req.params;
  try {
...
router.get("/connectors/:provider/oauth/start", async (req: Request, res: Response) => {
  const { provider } = req.params;
  ...
  const oauthUrls: Record<string, string> = {
    ...
    google: `https://accounts.google.com/o/oauth2/v2/auth?client_id=YOUR_GOOGLE_CLIENT_ID&response_type=code&scope=openid%20email%20profile&state=${state}`,

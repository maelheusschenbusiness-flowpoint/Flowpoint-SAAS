...
describe("J–L  Commission 35 % lifecycle", () => {
  ...
  it("J2 — Standard €29 first payment → commission €10.15 (35 %)", async () => {
    _dbRows.sellers.push({ id: "s1", seller_code: "SELLER-0042", status: "active" });
    await recordCommission({
      sellerId: "s1", orgId: "org_j2", customerEmail: "j2@test.com",
      plan: "standard", eligibleAmountCents: 2900, currency: "eur", attributionMethod: "ref_link",
    });
    const comm = _dbRows.seller_commissions.find(c => c.org_id === "org_j2");
    expect(comm?.commission_rate_bps).toBe(3500);
    expect(comm?.commission_amount_cents).toBe(1015);
  });

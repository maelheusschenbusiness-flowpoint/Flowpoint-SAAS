...
// Protected by ADMIN_KEY. Clean up with /api/admin/purge-ghost-accounts afterwards.
router.post("/admin/provision-test-org", async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    ...
    // 1. users row — minimal active user (no first_name/last_name in prod schema)
    await client.query(
      `INSERT INTO users (id, email, status, email_verified, auth_provider)
       VALUES ($1::uuid, $2, 'active', true, 'magic_link')
    ...
    // Resolve actual userId in case of conflict
    ...
    // 2. organizations row (is_internal_qa=FALSE, no stripe_customer_id)
    await client.query(
      `INSERT INTO organizations
         (id, name, slug, owner_user_id, owner_email, status, plan, subscription_status, is_internal_qa)
       VALUES ($1::uuid, $2, $3, $4, $5, 'active', $6, 'none', false)
       ON CONFLICT (id) DO NOTHING`,
      [orgId, email.split("@")[0] + "-cert-test",
       "cert-test-" + orgId.slice(0, 8),
       realUserId, email, plan],
    );
    ...
    // 4. Fresh 2-hour session (same schema as provision-qa-account)
    const sessionToken = `fp_cert_${randomBytes(32).toString("hex")}`;
    ...
    await client.query(
      `INSERT INTO user_sessions
       ...
       VALUES ($1, $2, $3, $4, 'owner', $5, NOW(), $6::uuid)
       ON CONFLICT DO NOTHING`,
      [sessionToken, realUserId, orgId, email, expiresAt, realUserId],
    );
    ...
    res.json({
      ...
      expiresAt:    expiresAt.toISOString(),
      note:         "Non-QA cert org — no Stripe customer yet; purge-ghost-accounts to clean up",
    });
  } catch (err) {
    await client.query("ROLLBACK").catch(() => {});
    res.status(500).json({ ok: false, error: safeErrMsg(err) });

...
router.post("/monitors", monitorCreateRateLimit, async (req, res) => {
  ...
  const [monitor] = await db.insert(monitorsTable).values({
    ...
    status: "up",
    uptime: 100,
    latency: 0, // populated after first health check
...
router.post("/monitors/:id/ping", async (req, res) => {
  ...
  // Re-validate at request time to defend against DNS rebinding and redirect-based SSRF.
  // Mirrors the same approach used by the scheduled cron checker.
  let initialParsed: URL;
  try {
    initialParsed = new URL(monitor.url);
  } catch {
    initialParsed = null as unknown as URL;
  }
  ...
  if (!pingBlocked) {
    try {
      ...
      const MAX_PING_REDIRECTS = 5;
      let currentUrl = monitor.url;
      let done = false;

      for (let hops = 0; hops <= MAX_PING_REDIRECTS; hops++) {
